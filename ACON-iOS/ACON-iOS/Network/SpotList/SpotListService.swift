    ...
    func postSpotList(requestBody: PostSpotListRequest,
        ...
        self.provider.request(.postSpotList(requestBody)) { result in
            ...
            case .success(let response):
                let networkResult: NetworkResult<PostSpotListResponse> = self.judgeStatus(statusCode: response.statusCode, data: response.data, type: PostSpotListResponse.self)
